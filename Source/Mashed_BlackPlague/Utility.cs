...
public static class Utility
{
    ...
    public static List<Pawn> TuurngaitInFactionList(Faction faction)
    {
        ...
        {
            if (enumerator.Current != null && enumerator.Current.RaceProps.Humanlike &&
                PawnIsTuurngait(enumerator.Current))
            {
    ...
    {
        return p.needs.mood != null && p.needs.mood.thoughts.memories.Memories.Any(x =>
            x.def == ThoughtDefOf.BlackPlague_TouchedVessel ||
            x.def == ThoughtDefOf.BlackPlague_TouchedVessel_Infected);
    }
    ...
    public static bool PawnIsNotValid(Pawn p)
    {
        ...
        if (p.health.hediffSet.GetFirstHediffOfDef(
                DefDatabase<HediffDef>.GetNamedSilentFail("ESCP_SloadThrallPassive")) != null)
        {
            return true;
        }
